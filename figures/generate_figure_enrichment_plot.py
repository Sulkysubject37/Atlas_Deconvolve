...
def create_enrichment_dot_plot(skip_api: bool = False):
    """
    Performs GO enrichment analysis on protein clusters and generates a dot plot
    of the most significant results.
    """
    if skip_api:
        print("Skipping API-based enrichment analysis as requested.")
        return
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate GO enrichment dot plot")
    parser.add_argument('--skip_api', action='store_true', help='Skip g:Profiler API call')
    args = parser.parse_args()

    create_enrichment_dot_plot(skip_api=args.skip_api)
